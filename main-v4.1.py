...
xlsx = pd.ExcelFile('data-v4.xlsx')
...
def run_randomforest(X_train, X_test, y_train, y_test, n_est, min_sp, dep, random_state=random.randint(0, 10000000)):
    ...
    decimal_part, integer_part = np.modf(predictions)
    rounded_decimal = np.where(decimal_part < 0.3, 0, np.where(decimal_part > 0.7, 1, 0.5))
    aligned_pred = integer_part + rounded_decimal
    aligned_mse = mean_squared_error(y_test, aligned_pred)
    aligned_rmse = np.sqrt(aligned_mse)
    aligned_mae = mean_absolute_error(y_test, aligned_pred)
    aligned_r2 = r2_score(y_test, aligned_pred)
    ...
    return {"MSE": mse, "RMSE": rmse, "MAE": mae, "R2": r2, # "Cla": score,
            ...
            "aligned MSE": aligned_mse, "aligned RMSE": aligned_rmse, "aligned MAE": aligned_mae, "aligned r2": aligned_r2,
            }
...
sheet_names = reversed(x for x in sheet_names)
...
for sheet in sheet_names:
    ...
    for n_est in tqdm([5, 10, 20, 50, 100, 200, 500]):
        for min_sp in tqdm([2, 3, 4, 5, 6, 7, 8, 9, 10], leave=False):
            for max_dep in tqdm([4, 5, 6, 7, 8, 9, 10], leave=False):
                for i in tqdm(range(20), leave=False, desc=str(BestScore)):
                    ...
                    logs.printf(f'{sheet}-RandomForest-n{n_est}-sp{min_sp}-dep{max_dep}-r{random_state}: ', {key: f"{value:.4f}" for key, value in results[sheet][f"RandomForest-n{n_est}-sp{min_sp}-dep{max_dep}-r{random_state}"].items()})
                    BestScore = max(BestScore, results[sheet][f"RandomForest-n{n_est}-sp{min_sp}-dep{max_dep}-r{random_state}"]['R2'])
    ...
    for hidden_neus in tqdm([(128, ), (256, ), (512, ), (1024, ),
        ...
        for i in tqdm(range(20), leave=False, desc=str(BestScore)):
            ...
            logs.printf(f'{sheet}-MLP-neu{hidden_neus}-r{random_state}: ', {key: f"{value:.4f}" for key, value in results[sheet][f"MLP-neu{hidden_neus}-r{random_state}"].items()})
            BestScore = max(BestScore, results[sheet][f"MLP-neu{hidden_neus}-r{random_state}"]['R2'])
    ...
    for n_est in tqdm([20, 50, 100, 200, 300, 500]):
        for i in tqdm(range(20), leave=False, desc=str(BestScore)):
            ...
            logs.printf(f'{sheet}-Bagging-nest{n_est}-r{random_state}: ', {key: f"{value:.4f}" for key, value in results[sheet][f"Bagging-nest{n_est}-r{random_state}"].items()})
            BestScore = max(BestScore, results[sheet][f"Bagging-nest{n_est}-r{random_state}"]['R2'])

            
# 输出每个表的预测准确率
json.dump(results, open(f"results-{VERSION}.json", 'w'))
